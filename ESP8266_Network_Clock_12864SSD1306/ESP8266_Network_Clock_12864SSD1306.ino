...
void setup()
{
  ...
  while (!Serial)
    continue;
  Serial.println("NTP Clock oled ");
  Serial.println("Designed by Zhang yuxing");
  initdisplay();
  u8g2.clearBuffer();
...
void loop()
{
  ...
  if (timeStatus() != timeNotSet)
  {
    ...
    { //时间改变时更新显示
      prevDisplay = now();
      oledClockDisplay(arr);
      
       Serial.printf("%d:%d:%d \n", arr[3],arr[4],arr[5]);
    
  if(arr[3] == int_hour && arr[4] == int_munite && arr[5]==int_second){
